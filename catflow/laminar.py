...
class Laminar():
    ...
    def submit_relaxation(
        ...
        if parameters is not None:
            ...
            atoms.info['calculator_parameters'] = data.get(
        ...
        for k, v in data.items():
            if isinstance(v, np.ndarray):
                utils.array_to_list(v)
                data[k] = v

        encoding = utils.atoms_to_encode(atoms)
        t0 = fireworks.PyTask(
            func='catflow.utils.encode_to_atoms',
            args=[encoding])

        # TODO: replace with a Custom Task
        t1 = fireworks.PyTask(
            func='catflow.tasks.get_potential_energy',
            args=[calculator],
    ...
    def bulk_relaxation(
        ...
        # TODO: replace with a Custom Task
        t1 = fireworks.PyTask(
            func='catflow.tasks.catflow_relaxation',
            stored_data_varname='trajectory')
